## 
[![Build Status](https://travis-ci.org/uwescience/.svg?branch=master)](https://travis-ci.org/uwescience/)
...
`/.py`. The code provided in this file is intentionally rather
...
to create a structure where when we type `import  as sb` in an
interactive Python session, the classes and functions defined inside of the
`.py` file are available in the `sb` namespace. For this to work, we
...
In the case of the  module, the main classes defined at the bottom of
...
Either way, you can create a `/data` folder in which you can
...
(typically, something like our `/tests/test_.py`). Within each
...
have one such test in `/tests/test_.py` called
...
the top-level directory of the repository (e.g. `/Users/arokem/code/`),
and type:
...
will see a message such as:
    ...
    /tests/test_.py:49: AssertionError
...
To document `` we use the [sphinx documentation
...
![RTD conf](https://github.com/uwescience//blob/master/doc/_static/RTD-advanced-conf.png)
...
A `/version.py` contains all of the information needed for the
installation and for setting up the [PyPI
page](https://pypi.python.org/pypi/) for the software. This
...
installation of the software dependencies. For ``, we use the
...
[here](https://github.com/uwescience//blob/master/scripts/Figure1.ipynb).
...
button on the front page of the "" repository.
...
`/.py` to be called `smallish/smallish.py`

	git mv  smallish
	git mv smallish/.py smallish/smallish.py
	git mv smallish/tests/test_.py smallish/tests/test_smallish.py

Make a commit recording these changes. Something like:

	git commit -a -m"Moved names from `` to `smallish`"

You will probably want to remove all the example data:

	git rm smallish/data/*
	git commit -a -m"Removed example `` data"

Possibly, you will want to add some of your own data in there.

You will want to edit a few more places that still have `` in them. Type
the following to see where all these files are:

	git grep 

You can replace `` for `smallish` quickly with:

	git grep -l '' | xargs sed -i 's//smallish/g'
...
Edit all the mentions of `` in the `/__init__.py` file, and in
the `/version.py` file as well.
